...
\onslide<7,8> {To authenticate it ask for master password + device}
\end{block}
...
\onslide<3,10> {C++ library, cross-platform, elegant design}
\end{block}

\begin{block}<4,5,10>{SQLite: DataBase management}
\onslide<5,10> {Self-contained, written in C, Transactional}
\end{block}

\begin{block}<6,7,10>{PwGen: Password generator}
\onslide<7,10> {Configurable, random or readable}
\end{block}

\begin{block}<8,9,10>{zxcvbn: Password strength estimator}
\onslide<9,10> {Dictionaries, keyboard patterns, sequences, years}
\end{block}
\end{frame}
...
\only<2->{
 \begin{textblock}{4.5}(10,3)
  \only<2>{\footnotesize \color{black} Device discovery \\ Open Connection\\ \hfill  }
  \only<3>{\footnotesize \color{black} Authentication \\ Get Algorithms\\ \hfill}
  \only<4>{\footnotesize \color{black} SEfile \\ secureSQLite \\ SElink}
  \only<5>{\footnotesize \color{black} secureSQLiteBrowser \\ SEcubeWallet\\ \hfill}
 \end{textblock}
}
\end{frame}
